        ...
        unsigned char *data)
{
   ...
   // unsigned char ans[2];
    //i2c_read(st->hw->addr, st->reg->bank_sel, 2, ans);
    //printf("WRITE ADDR: %d %d\n",ans[0],ans[1]);
    if (i2c_write(st->hw->addr, st->reg->bank_sel, 2, tmp))
        return -4;
// i2c_read(st->hw->addr, st->reg->bank_sel, 2, ans);
  //  printf("AFTER WRITE ADDR: %d %d\n",ans[0],ans[1]);
...
void mpu_test(){
    ...
    i2c_read(st->hw->addr, st->reg->bank_sel, 2, ans);
    //i2c_read(st->hw->addr, 0x1a, 2, ans);
    printf("%d WRITE ADDR: %d %d\n",ans[2],ans[0],ans[1]);
